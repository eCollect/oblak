...
const buildRoute = (route, operations, api, wire, uri) => {
	if (operations[oblakOperation])
		operations = normalizers.routeDefinition(operations[oblakOperation](true, api));

	Object.entries(operations).forEach(([method, operation]) => {
		operation = normalizers.operationDefintion(buildOperation(operation, api));
...
const buildRouter = (rest, { Router }, api, wire, uri = '/') => {
	...
	Object.entries(routes).forEach(([route, operations]) => {
		...
		buildRoute(router.route(route), normalizers.routeDefinition(operations), api, wire, urlJoin(uri, route));
	});
