...
class JobRunner {
	...
	async init({ path, config }) {
		...
		this._stages = this._execute.length;
	...
	async execute(job) {
		...
		try {
			...
			if (stage < this._stages)
				for (let i = stage; i < this._stages; i++) {
					this._execute[i](lockedJob, this.container, api);
			...
			this.logger.error(e, lockedJob, 'Error while executing.');
