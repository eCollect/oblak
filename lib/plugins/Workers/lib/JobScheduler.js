...
class JobScheduler extends EventEmitter {
	constructor(app, store, worker) {
		super();
		this.app = app;
		this.store = store;
		this.workerAddress = worker;
	}
	...
	async getNextParitionedJob() {
		return this.store.getNextPartitionedJob(this.workerAddress);
	}

	// eslint-disable-next-line class-methods-use-this
	shouldJobBeExecuted(job) {
		return job.execution.getTime() <= Date.now();
	}
