...
class JobScheduler extends EventEmitter {
	...
	async getOrAddJob(job) {
		const { id = 'mitko', payload = {}, metadata = {}, envelope, execution } = job;
		if (!id)
	...
	static normalizeExecution({ notBefore = MIN_DATE, notAfter = MAX_DATE, partitionKey = null, cron = null } = {}) {
		if (cron)
	...
	static parseCron(notBefore = MIN_DATE, { interval = '', tz = null, times = 0  } = {}) {
		if (!interval)
