...
class OblakWire extends Base {
	...
	read(readmodel, metadata = {}, query = {}, one = false) {
		const method = one ? 'readOne' : 'readNew';
	...
	readStream(readmodel, metadata, query = {}, one = false) {
		...
		const incomingStream = this.readmodelStore[method](readmodel, {}, query);
	...
	subscribeToEvents(where = {}) {
		...
		stream.once('finish', () => {
			this.app.notificationbus.incoming.unpipe(stream);
			this.app.notificationbus.incoming.resume();
		});
		...
		// pipe the damn thing
		this.app.notificationbus.incoming.pipe(stream);
