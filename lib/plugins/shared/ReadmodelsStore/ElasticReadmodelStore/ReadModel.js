...
class ElasticGetQuery {
	...
	stream() {
		return new ElasticGetStream(this.client, this.index, this.id).safePipe(new MapSourceStream());
	}
}
...
class ElasticRawQuery {
	...
	stream() {
		return new ElasticReadStream(this.client, this.index, this.body).safePipe(new MapHitStreamMinimal());
	}
}
