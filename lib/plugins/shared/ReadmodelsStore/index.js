...
module.exports = class ReadmodelsStore {
	...
	async init({ readmodels }) {
		await Object.entries(readmodels).reduce(async (prm, [type, models]) => {
			...
			const repositoryOpitons = storeOptions(this.app.config.denormalizers[type], this.app);

			if (!typedStores[repositoryOpitons.type])
				return;

			this.stores[type] = await new typedStores[repositoryOpitons.type](this.app, repositoryOpitons).init(models);
		}, Promise.resolve());
	...
	query(type, modelName, id) {
		if (!(type in this.stores))
			throw new Error(`No readmodel store of type ${type} initilized.`);
		return this.stores[type].query(modelName, id);
	}
};
