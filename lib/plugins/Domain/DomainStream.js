...
const buildApi = require('./api/buildDomainClass');
...
module.exports = class DomainStream extends OblakTransform {
	init({ domain, validationService }, wire) {
		...
		// const domainApi = buildApi(wire, () => this.cqrsDomain);

		const DomainClass = buildApi((context, aggregate, id) => new Promise((res, rej) => {
			const ctx = this.cqrsDomain.tree.getContext(context);
			...
			agg.defaultCommandHandler.loadAggregate({
				...
				if (err)
					return rej(err);
				return res(agg.attributes);
			});
		}), { domain });

		const domainApi = () => ({
			...wire.wireApi.get(),
			getDomain: () => new DomainClass(),
		});
	...
	async _transform(command, _, done) {
		...
		this.cqrsDomain.handle(serializedCommand, (error, events) => {
			if (error) {
				this.push(this.app.events.commandRejected(command, JSON.parse(JSON.stringify(error.more || error))));
				command.reject();
				return; // Handle error
			}
			command.ack();
			events.sort((a, b) => a.aggregate.revision - b.aggregate.revision).map(e => this.push(new this.app.Event({ type: this.app.Event.EVENT_TYPES.DOMAIN, ...e })));
		});
		done(null);
	}
