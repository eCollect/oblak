...
module.exports = class SagaStream extends OblakTransform {
	...
	async getToLatestPosition() {
		const currentPosition = this.positionGuard ? await this.positionGuard.check(this.wire.name, {}) : null;
		if (!currentPosition)
			return;
		const eventStream = await this.wire.eventstore.getReplay({ fromPosition: currentPosition });
		for await (const { payload: evt } of eventStream) {
			if (!evt.metadata.position)
				continue;
			evt.fullname = `domain.${evt.context}.${evt.aggregate.name}.${evt.name}`;
			this.logger.info(`Handling missed event ${evt.fullname} / ${evt.metadata.position}`);
	...
	async _transform(evt, _, done) {
		...
		if (currentPosition && position && currentPosition + 1 !== position) {
			if (position > currentPosition) {
				...
				await this.getToLatestPosition();
			}
			evt.ack();
			return done(null);
		}
		...
		try {
			return await this._handleEvent(serializedEvent);
		} catch (e) {
			return done(e);
		}
	}
