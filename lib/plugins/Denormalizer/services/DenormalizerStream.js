...
module.exports = class DenormalizerStream extends OblakTransform {
	...
	async init(oblak, wire) {
		...
		this.denormalizer.onNotification(n => this.pushNotification(n)).init();
	}
	...
	async _replayEvents() {
		return new Promise((resolve, reject) => {
			this.eventstore.store.connect(async (error) => {
				...
				const eventsStream = this.eventstore.streamEvents({}).on('error', reject);
				const replayStream = this.denormalizer.getReplayStream().on('error', reject);
				return eventsStream.pipe(replayStream).on('finish', resolve);
			});
		});
	}
