		...
		return new Promise(async (resolve, reject) => {
			...
			return pipeline(eventsStream, replayStream, (err) => {
				if (progress) {
					...
					progress.done(`${this.type} rebuild ${stats.all.count} events in ${this.stats.all.took}.`);
					Object.entries(stats).filter(([key]) => key !== 'all').sort(([, time1], [, time2]) => time2.max - time1.max).slice(0, 10)
						.forEach(([fullname, s]) => konzola.info(`[${fullname}]: ${s.count} events, avg: ${s.avg}ms, max: ${s.max}ms`));
				}
