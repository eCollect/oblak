...
module.exports = class Collection {
	...
	async handle(listModelStore, apiBuilder, context) {
		...
		for (const { query, actions, settings } of this._reactions[context.domainEvent.fullname]) {
			...
			if (typeof settings.shouldHandle === 'function' && !(await settings.shouldHandle(context.domainEvent)))
				continue;

			const baseQuery = query(context.domainEvent);
			const modelStream = listModelStore.findViewmodel(this.name, buildQuery(baseQuery)).stream();
