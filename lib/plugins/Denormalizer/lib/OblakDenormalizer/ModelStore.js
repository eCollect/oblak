...
class InMemoryFindQuery {
	...
	stream() {
		throw new Exception('Not possible in replay mode :(');
...
class InMemoryGetQuery {
	...
	stream() {
		const vm = this.collection.get(id, true);
		if (!vm)
...
class InMemoryQueryExecutor {
	...
	raw(raw, metadata, options) {
		return new InMemoryQuery(raw, metadata, options, this);
	}
...
class InMemoryCollection {
	constructor (modelName, backupStore) {
	...
	getBackupQueryExecutor() {
		return this._backupStore.getQueryExecutor(this._modelName);
	}
	...
	async flush() {
		...
		this._flushed = true;
		const fulshed = await this._backupStore.batchSaveViewModels(this._modelName, this._buffer);
...
class ModelStore {
	constructor({
		...
		Object.keys(dbModelStore.collections || dbModelStore.indexes).forEach((col) => {
			this.buffers[col] = new InMemoryCollection(col, dbModelStore);
		});
	}
	...
	async saveViewmodel(model, vm) {
		...
		// return this.dbModelStore.saveViewmodel(model, vm)
	}
	...
	async flush() {
		return Object.values(this.buffers).map(b => b.flush(true));
	}
