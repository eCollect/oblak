...
module.exports = class ListDenormalizer {
	...
	async clear() {
		...
		this._collections.map(collection => this.eventSequencer.registerModel(this.type, collection.name, { position: 0, id: null }));
	}

	async handle(evt) {
		const context = new HandleContext(evt, this, { isReplay: false });
	...
	async replay(evt, collections = this._collections) {
		const context = new HandleContext(evt, this, { isReplay: true });
		await Promise.all(collections.map(collection => handleReplay(collection, context, this)));
	...
	getReplayStream(collections) {
		return new ReplayStream(this, collections);
	}
};
