...
class MapHit extends Transform {
	constructor() {
...
const readmodelsListMiddleware = ({
	...
	const str = req.getApp().wire.read(readmodel, req.oblakMetadata, parse(req.query));
	// str.then(resp => res.json(resp), next);
	str.pipe(new MapHit()).pipe(new JsonListResultsStream(res));
};
